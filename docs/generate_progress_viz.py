...
fig.text(
    0.5, 0.948,
    "Branch: main  ·  Version: 1.1.0  ·  April 2026",
    ha="center", va="center",
...
for i, (pr_num, date, label, color, size_tier) in enumerate(prs):
    ...
    ax3.text(x, y_base - 0.22, label, ha="center", va="top",
             color=WHITE, fontsize=7.8, multialignment="center",
...
ax3.set_title("Pull Request Merge Timeline  (6 PRs merged to main)", color=WHITE,
              fontsize=12, fontweight="bold", pad=10)
...
for xi, (total, fail) in enumerate(zip(test_totals, failures)):
    ax4.annotate(str(total), (xi, total), textcoords="offset points",
                 xytext=(0, 8), ha="center", color=GREEN, fontsize=8.5, fontweight="bold")
...
for label, effort, impact, size, color in gaps:
    ax5.scatter(effort, impact, s=size, color=color, alpha=0.75,
                edgecolors=DARK_BG, linewidth=2, zorder=3)
    ax5.text(effort, impact + 0.28, label, ha="center", va="bottom",
             color=WHITE, fontsize=8, multialignment="center")
...
# Background
ax6.add_patch(FancyBboxPatch((0.02, bar_y), 0.96, bar_h,
    boxstyle="round,pad=0.01", facecolor=BORDER, edgecolor=BORDER, transform=ax6.transAxes))

# Fill
fill_w = 0.96 * pct
ax6.add_patch(FancyBboxPatch((0.02, bar_y), fill_w, bar_h,
    boxstyle="round,pad=0.01", facecolor=GREEN, edgecolor=GREEN, alpha=0.85, transform=ax6.transAxes))

# Amber gap segment
gap_start = 0.02 + fill_w
gap_w = 0.96 * (1/12)  # admin UI partial
ax6.add_patch(FancyBboxPatch((gap_start, bar_y), gap_w, bar_h,
    boxstyle="round,pad=0.01", facecolor=AMBER, edgecolor=AMBER, alpha=0.6, transform=ax6.transAxes))

ax6.text(0.5, bar_y + bar_h + 0.12, f"Overall Platform Completeness:  {total_complete}/{total_layers} Layers  ({pct*100:.0f}%)",
         ha="center", va="bottom", transform=ax6.transAxes,
         fontsize=13, fontweight="bold", color=WHITE)

ax6.text(0.5, bar_y - 0.22,
