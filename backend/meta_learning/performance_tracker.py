...
class PerformanceTracker:
    ...
    def __init__(self):
        ...
        self._complexity_stats: Dict[str, Dict[str, Any]] = defaultdict(lambda: {
            "count": 0,
            "success_count": 0,
            "total_duration": 0.0,
            "total_cost": 0.0
    ...
    def _update_agent_performance(self, outcome: MissionOutcome):
        ...
        if agent_outcomes:
            perf.average_quality = sum(o.quality_score for o in agent_outcomes) / len(agent_outcomes)
    ...
    def _calculate_trend(self, agent_id: str) -> str:
        ...
        recent_success_rate = sum(1 for o in recent if o.outcome == OutcomeType.SUCCESS) / len(recent)
        previous_success_rate = sum(1 for o in previous if o.outcome == OutcomeType.SUCCESS) / len(previous)
    ...
    def get_leaderboard(self, metric: str = "success_rate", limit: int = 10) -> List[AgentPerformance]:
        """
    ...
    def get_learning_insights(self, agent_id: str) -> Dict[str, Any]:
        ...
        insights = {
            ...
            "quality": {
                "avg_score": perf.average_quality
            },
            "recommendations": []
        }
        ...
        if perf.success_rate < 0.7:
            insights["recommendations"].append({
                "type": "success_rate",
                "message": "Success rate is below 70%. Consider adjusting mission complexity or agent configuration.",
                "priority": "high"
            })
        ...
        if perf.avg_cost_per_mission > 5.0:
            insights["recommendations"].append({
                "type": "cost",
                "message": "Average cost per mission is high. Consider using more efficient models.",
                "priority": "medium"
            })
        ...
        if perf.recent_trend == "declining":
            insights["recommendations"].append({
                "type": "trend",
                "message": "Performance is declining. Review recent failures and adjust strategy.",
                "priority": "high"
            })
