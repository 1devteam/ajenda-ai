...
class TrendAnalyzer:
    ...
    def get_risk_score_trends(
        ...
        forecast_30d, forecast_60d, forecast_90d, confidence = self._forecast(time_series, [30, 60, 90])
    ...
    def get_approval_volume_trends(self, days: int = 30) -> TrendData:
        ...
        if not time_series:
            ...
            requiring_approval = sum(
                1 for asset in assets
                if self.risk_engine.get_score(asset.asset_id) and
                self.risk_engine.get_score(asset.asset_id).tier in [RiskTier.MEDIUM, RiskTier.HIGH, RiskTier.CRITICAL]
            )
