...
"""
from sqlalchemy import Column, String, DateTime, Float, Integer, Boolean, ForeignKey, JSON, Text, Enum as SQLEnum
...
class GovernanceAsset(Base):
    ...
    lineage_events = relationship("GovernanceLineageEvent", back_populates="asset", cascade="all, delete-orphan")
    risk_scores = relationship("GovernanceRiskScore", back_populates="asset", cascade="all, delete-orphan")
    audit_events = relationship("GovernanceAuditEvent", back_populates="asset", cascade="all, delete-orphan")
    approvals = relationship("GovernanceApproval", back_populates="asset", cascade="all, delete-orphan")
...
class GovernancePolicy(Base):
    ...
    evaluations = relationship("GovernancePolicyEvaluation", back_populates="policy", cascade="all, delete-orphan")
...
class GovernanceAuditEvent(Base):
    ...
    event_category = Column(String(50), nullable=False, index=True)  # access, modification, compliance, etc.
...
class GovernanceApproval(Base):
    ...
    status = Column(SQLEnum(ApprovalStatus), nullable=False, index=True, default=ApprovalStatus.PENDING)
...
class GovernanceComplianceFinding(Base):
    ...
    # Finding details
    check_type = Column(String(50), nullable=False, index=True)  # policy, regulatory, security, etc.
