...
class AuditRepository(BaseRepository[GovernanceAuditEvent]):
    ...
    def get_by_tenant(
        ...
        """
        return self.db.query(GovernanceAuditEvent).filter(
            GovernanceAuditEvent.tenant_id == tenant_id
        ).order_by(desc(GovernanceAuditEvent.timestamp)).limit(limit).offset(offset).all()
    ...
    def get_by_asset(
        ...
        """
        return self.db.query(GovernanceAuditEvent).filter(
            GovernanceAuditEvent.asset_id == asset_id
        ).order_by(desc(GovernanceAuditEvent.timestamp)).limit(limit).offset(offset).all()
    ...
    def get_failed_events(
        ...
        ).order_by(desc(GovernanceAuditEvent.timestamp)).limit(limit).all()
    ...
    def search_events(
        ...
        return query.order_by(desc(GovernanceAuditEvent.timestamp)).limit(limit).offset(offset).all()
    ...
    def get_statistics(
        ...
        # Total events
        total = self.db.query(GovernanceAuditEvent).filter(
            and_(
                GovernanceAuditEvent.tenant_id == tenant_id,
                GovernanceAuditEvent.timestamp >= cutoff
            )
        ...
        severity_counts = self.db.query(
            GovernanceAuditEvent.severity,
            func.count(GovernanceAuditEvent.id)
        ).filter(
            and_(
                GovernanceAuditEvent.tenant_id == tenant_id,
                GovernanceAuditEvent.timestamp >= cutoff
            )
        ).group_by(GovernanceAuditEvent.severity).all()
        ...
        ).group_by(GovernanceAuditEvent.outcome).all()
        ...
        category_counts = self.db.query(
            GovernanceAuditEvent.event_category,
            func.count(GovernanceAuditEvent.id)
        ).filter(
            and_(
                GovernanceAuditEvent.tenant_id == tenant_id,
        ...
        ).group_by(GovernanceAuditEvent.event_category).all()
    ...
    def detect_anomalies(
        ...
        """
        cutoff = datetime.utcnow() - timedelta(hours=hours)
        count = self.db.query(GovernanceAuditEvent).filter(
            and_(
                GovernanceAuditEvent.tenant_id == tenant_id,
                GovernanceAuditEvent.actor_id == actor_id,
                GovernanceAuditEvent.timestamp >= cutoff
