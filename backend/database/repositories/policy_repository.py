...
class PolicyRepository(BaseRepository[GovernancePolicy]):
    ...
    def get_by_status(
        ...
        """
        return self.db.query(GovernancePolicy).filter(
            and_(
                GovernancePolicy.tenant_id == tenant_id,
                GovernancePolicy.status == status
            )
        ).order_by(desc(GovernancePolicy.priority)).limit(limit).offset(offset).all()
...
class PolicyEvaluationRepository(BaseRepository[GovernancePolicyEvaluation]):
    ...
    def get_asset_evaluations(
        ...
        """
        return self.db.query(GovernancePolicyEvaluation).filter(
            GovernancePolicyEvaluation.asset_id == asset_id
        ).order_by(desc(GovernancePolicyEvaluation.evaluated_at)).limit(limit).all()
    ...
    def get_cached_evaluation(
        ...
        ).order_by(desc(GovernancePolicyEvaluation.evaluated_at)).first()
    ...
    def delete_asset_evaluations(self, asset_id: str) -> int:
        ...
        """
        count = self.db.query(GovernancePolicyEvaluation).filter(
            GovernancePolicyEvaluation.asset_id == asset_id
        ).delete()
        self.db.commit()
        return count
