...
class ApprovalRepository(BaseRepository[GovernanceApproval]):
    ...
    def get_by_requester(
        ...
        return query.order_by(desc(GovernanceApproval.requested_at)).limit(limit).offset(offset).all()
    ...
    def get_by_approver(
        ...
        Returns:
            List of GovernanceApproval instances
        """
        return self.db.query(GovernanceApproval).filter(
            and_(
                GovernanceApproval.tenant_id == tenant_id,
                GovernanceApproval.status == status,
                GovernanceApproval.approvers.contains([approver_id])
            )
        ).order_by(GovernanceApproval.requested_at).limit(limit).offset(offset).all()
    ...
    def expire_old_requests(
        ...
        """
        now = datetime.utcnow()
        expired = self.db.query(GovernanceApproval).filter(
            and_(
                GovernanceApproval.tenant_id == tenant_id,
                GovernanceApproval.status == ApprovalStatus.PENDING,
    ...
    def get_statistics(
        ...
        # Total requests
        total = self.db.query(GovernanceApproval).filter(
            and_(
                GovernanceApproval.tenant_id == tenant_id,
        ...
        for status in ApprovalStatus:
            count = self.db.query(GovernanceApproval).filter(
                and_(
                    GovernanceApproval.tenant_id == tenant_id,
                    GovernanceApproval.status == status,
                    GovernanceApproval.requested_at >= cutoff
        ...
        approved = self.db.query(GovernanceApproval).filter(
            and_(
                GovernanceApproval.tenant_id == tenant_id,
        ...
        if approved:
            total_time = sum([
                ...
                for a in approved if a.approved_at
            ])
