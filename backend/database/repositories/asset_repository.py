...
class AssetRepository(BaseRepository[GovernanceAsset]):
    ...
    def get_by_owner(
        ...
        Returns:
            List of GovernanceAsset instances
        """
        return self.db.query(GovernanceAsset).filter(
            and_(
                GovernanceAsset.owner_id == owner_id,
                GovernanceAsset.tenant_id == tenant_id
            )
        ).order_by(GovernanceAsset.created_at.desc()).limit(limit).offset(offset).all()
    ...
    def get_by_risk_tier(
        ...
        ).order_by(GovernanceAsset.risk_score.desc()).limit(limit).offset(offset).all()
    ...
    def get_high_risk_assets(
        ...
        ).order_by(GovernanceAsset.risk_score.desc()).limit(limit).all()
    ...
    def get_by_compliance_status(
        ...
        ).order_by(GovernanceAsset.updated_at.desc()).limit(limit).offset(offset).all()
    ...
    def get_dependents(self, asset_id: str) -> List[GovernanceAsset]:
        ...
        """
        return self.db.query(GovernanceAsset).filter(
            GovernanceAsset.dependencies.contains([asset_id])
        ).all()
    ...
    def get_statistics(self, tenant_id: str) -> Dict[str, Any]:
        ...
        for risk_tier in RiskTier:
            count = self.db.query(GovernanceAsset).filter(
                and_(
                    GovernanceAsset.tenant_id == tenant_id,
                    GovernanceAsset.risk_tier == risk_tier
                )
