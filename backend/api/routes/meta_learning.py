...
from backend.meta_learning import (
    get_tracker,
    get_engine,
    MissionOutcome,
    OutcomeType
...
async def get_agent_performance(agent_id: str):
    ...
    if not perf:
        raise HTTPException(status_code=404, detail=f"Agent {agent_id} not found or has no performance data")
...
async def get_leaderboard(
    ...
    """
    valid_metrics = ["success_rate", "avg_cost_per_mission", "avg_duration_per_mission", "average_quality"]
...
async def auto_optimize_agent(agent_id: str):
    ...
    return {
        ...
        "configuration": {
            ...
            "max_cost_per_mission": round(config.max_cost_per_mission, 2) if config.max_cost_per_mission else None,
            "target_duration": round(config.target_duration, 2) if config.target_duration else None,
...
async def get_agent_configuration(agent_id: str):
    ...
    if not config:
        raise HTTPException(
            status_code=404,
            detail=f"No optimized configuration found for agent {agent_id}. Run /optimize/{agent_id} first."
        )
...
async def get_meta_learning_stats():
    ...
    return {
        ...
        "agents_ready_for_optimization": len([p for p in all_perf.values() if p.total_missions >= 10])
    }
