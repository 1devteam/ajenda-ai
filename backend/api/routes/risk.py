...
async def get_risk_score(asset_id: str):
    ...
    try:
        ...
        if not risk_score:
            raise HTTPException(status_code=404, detail=f"Risk score not found for asset {asset_id}")
...
async def check_approval_required(asset_id: str, operation: str = Query(..., description="Operation")):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error checking approval requirement: {str(e)}")
...
@router.get("/mitigation-strategies/{risk_tier}", response_model=List[MitigationStrategyResponse])
async def get_mitigation_strategies(risk_tier: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving mitigation strategies: {str(e)}")


@router.get("/mitigation-strategies/asset/{asset_id}", response_model=List[MitigationStrategyResponse])
