...
# JSON type imported from sqlalchemy
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('asset_type', sa.Enum('agent', 'tool', 'model', 'vector_db', 'dataset', 'workflow', name='assettype'), nullable=False, index=True),
        sa.Column('status', sa.Enum('active', 'inactive', 'archived', 'suspended', 'under_review', name='assetstatus'), nullable=False, index=True),
        ...
        sa.Column('risk_tier', sa.Enum('unacceptable', 'high', 'limited', 'minimal', name='risktier'), nullable=True, index=True),
        sa.Column('risk_score', sa.Float, nullable=True),
        sa.Column('compliance_status', sa.Enum('compliant', 'non_compliant', 'needs_review', 'exempted', name='compliancestatus'), nullable=True, index=True),
    ...
    op.create_table(
        ...
        sa.Column('id', sa.String(50), primary_key=True, index=True),
        sa.Column('asset_id', sa.String(50), sa.ForeignKey('governance_assets.id', ondelete='CASCADE'), nullable=False, index=True),
        sa.Column('tier', sa.Enum('unacceptable', 'high', 'limited', 'minimal', name='risktier'), nullable=False, index=True),
    ...
    op.create_table(
        ...
        sa.Column('name', sa.String(255), nullable=False),
        sa.Column('description', sa.Text, nullable=True),
        sa.Column('status', sa.Enum('draft', 'active', 'inactive', 'archived', name='policystatus'), nullable=False, index=True),
    ...
    op.create_table(
        ...
        sa.Column('required_authority', sa.Enum('guest', 'user', 'operator', 'admin', 'compliance_officer', name='authoritylevel'), nullable=False, index=True),
        sa.Column('status', sa.Enum('pending', 'approved', 'rejected', 'escalated', 'expired', name='approvalstatus'), nullable=False, index=True),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('compliant', 'non_compliant', 'needs_review', 'exempted', name='compliancestatus'), nullable=False, index=True),
...
def downgrade() -> None:
    ...
    op.execute("""
        ...
        DROP TYPE IF EXISTS authoritylevel;
